...
> **See [auth-session-database.md](auth-session-database.md) Layer 2** for the canonical implementation plan — this file is a high-level overview. auth-session-database.md has pinned API decisions for the pool middleware, `after_send` pool checkin, context module convention, migration format, test sandbox, and the `user_tokens` table schema generated by `forge gen.auth session`.
