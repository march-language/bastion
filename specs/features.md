...
| Auth middleware | `auth.march` (security/) | `load_current_user/2` + `load_current_user_with_remember/3`; `require_auth` (redirects to /login); `authenticated` sugar; `log_in` with optional remember-me cookie; `log_out` with optional token deleter; `redirect_after_login` |
...
| Structured logging | `logger.march` | `Logger.debug/info/warn/error(msg, meta)` + `*_conn/3` helpers; logfmt (`HH:MM:SS.mmm [level] msg  k=v`) or JSON one-liner, selected by an explicit `Logger.configure(level, format)` call — **not** auto-detected from MARCH_ENV |
| Request ID middleware | `middleware.march` | `Middleware.request_id`: `Crypto.random_hex(16)` for new IDs, echoes `x-request-id` response header; `Middleware.logger` logs via `Logger.log_with(Logger.Info, …)` with request_id in meta |
...
| Dev error overlay (500 page) | `error_overlay.march` | `Bastion.ErrorOverlay`: static 500 error page + `router_plug/1`. **Stub:** `rescue_errors/1` is a no-op and the crash-site REPL (`/_bastion/debug/:id`) returns 404 — both blocked on a `HttpServer.try_call` panic-recovery primitive in the March stdlib |
...
| Cmd abstraction (WASM → JS) | `lib/runtime/cmd.march`, `priv/js/march-islands.js`, `priv/js/wasm-bridge.js` | `Bastion.Cmd` type + constructors; `executeCmd` runtime for `HttpGet/Post`, `After/Every`, `Focus/Blur`, `PushUrl/ReplaceUrl`, `StoreLocal/LoadLocal/RemoveLocal`, `ChannelPush`, `Batch`; `WasmIslandModule.updateWithCmd()` reads cmd via `march_island_last_cmd` sidecar export |
| JS FFI layer (WASM → browser) | `lib/view/js.march`, `priv/js/wasm-bridge.js` | `Bastion.JS`: `call`, `global`, `eval`, `query_selector`, `get/set/remove_attribute`, `add_event_listener`, `string_val/int_val/bool_val/json_val`, `to_string/to_int`; JS handle table in wasm-bridge.js; `extern "bastion" "js_*"` imports; late-bound memory reference |
...
| OpenTelemetry export (partial) | `otel.march` | `Bastion.OpenTelemetry`: `default_config/1`, `start/1`; attaches to "stop"/"exception" events and builds valid OTLP resourceSpans JSON. **Stub:** the actual HTTP POST is not implemented (`export_span` discards the payload) — blocked on a stdlib async `HttpClient` |
| Island test helpers | `lib/testing/test_island.march`, `test/counter_island.march`, `examples/counter_island.march` | `Bastion.Test.Island`: `render_island/3`, `update_island/3`, `assert_renders_contains/4`, `cmd_type/1`, `assert_no_server_event_attrs/1`; canonical `CounterIsland` example (init/update/render/encode/decode) |
...
| Telemetry event coverage | `lib/channel_server.march`, `lib/depot_middleware.march`, `lib/bastion_server.march` | `["bastion","channel","message"]` span in handle_in; `query_with_telemetry/3` wrapping `exec_sql`; `["bastion","endpoint","exception"]` is **specced but not emitted** — `safe_call_plug` is currently a no-op pending a `HttpServer.try_call` catch primitive |
...
| OpenTelemetry export | [telemetry.md](telemetry.md) | **Partial** — `otel.march` attaches to events and builds valid OTLP JSON, but `export_span` discards the payload (no HTTP POST yet) | Stdlib async `HttpClient` (same blocker as the panic-recovery primitive) |
